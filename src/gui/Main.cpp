...
int main(int argc, char **argv) {
    ...
    loopi(0, Simplify::vertices.size()) { obj.objects[0].points[i] = Simplify::vertices[i].p; }
    int ofs = 0;
...
void DrawScene() {
    ...
    glLoadIdentity();
    gluPerspective(60.0, (GLfloat)screen.window_width / (GLfloat)screen.window_height, 0.1,
                   10000.0);
    ...
    if ((frame & 31) == 0) {
        char txt[1000];
        int tcount = obj.objects[0].triangles.size();
        sprintf(txt, "Triangles:%d FPS: %f MT/S: %f", tcount, fps, float(tcount) * fps / 1000000);
        glutSetWindowTitle(txt);
    }

    printf("pos %2.2f %2.2f %2.2f rot %2.2f %2.2f %2.2f \r", screen.pos.x, screen.pos.y,
           screen.pos.z, screen.rot.x, screen.rot.y, screen.rot.z);

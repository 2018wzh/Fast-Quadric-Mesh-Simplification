  ...
  public:
    ...
    void draw(Material *materials) {
        ...
        loopi(0, subgroups.size()) {
            ...
            static Bmp default_bmp("../data/white.png");
            static int tex_default = CoreTextureFromBmp(default_bmp);
            ...
            glBindTexture(GL_TEXTURE_2D, tex_amb);

            glEnable(GL_CULL_FACE);
            glCullFace(GL_BACK);

            tri_shader.begin();
            tri_shader.setUniformMatrix4fv(const_cast<char *>("projectionMatrix"), 1, 0,
                                           Projection);
            tri_shader.setUniformMatrix4fv(const_cast<char *>("modelViewMatrix"), 1, 0, Modelview);
            // tri_shader.setUniform3f("lightPos",0,-10,0);
            tri_shader.setUniform4f(const_cast<char *>("ambient"), am.x, am.y, am.z, 0);
            tri_shader.setUniform4f(const_cast<char *>("diffuse"), di.x, di.y, di.z, 0);
            tri_shader.setUniform4f(const_cast<char *>("specular"), sp.x, sp.y, sp.z, 0);
  ...
  public:
    ...
    void print_materials() {
        for (uint i = 0; i < materials.size(); i++) {
            ...
            printf("  Ambient RGB %2.2lf %2.2lf %2.2lf\n", materials[i].ambient.x,
                   materials[i].ambient.y, materials[i].ambient.z);
            printf("  Specular RGB %2.2lf %2.2lf %2.2lf\n", materials[i].specular.x,
                   materials[i].specular.y, materials[i].specular.z);
            printf("  Diffuse RGB %2.2lf %2.2lf %2.2lf\n", materials[i].diffuse.x,
                   materials[i].diffuse.y, materials[i].diffuse.z);
    ...
    void print_objects() {
        for (uint i = 0; i < objects.size(); i++) {
            ...
            printf("  - Center   : %2.3lf %2.3lf %2.3lf\n", objects[i].translation.x,
                   objects[i].translation.y, objects[i].translation.z);
            printf("  - BB max   : %2.3lf %2.3lf %2.3lf\n", objects[i].bb_max.x,
                   objects[i].bb_max.y, objects[i].bb_max.z);
            printf("  - BB min   : %2.3lf %2.3lf %2.3lf\n", objects[i].bb_min.x,
                   objects[i].bb_min.y, objects[i].bb_min.z);
        }
    }
    int get_object_index(std::string &name) {
        for (uint i = 0; i < objects.size(); i++)
            if (name.compare(objects[i].name) == 0)
                return i;
    ...
    std::string get_pure_filename(std::string filename) {
        ...
        // filename.substr(pos1,pos3-pos1).c_str());
    ...
    void load_mtl(const char *filename, std::string prefix) {
        ...
        while (fgets(line, 1000, fn) != NULL) {
            ...
            case 'm':
                ...
                break;
            case 'K':
                if (sscanf(line, "Kd %lf %lf %lf", &color.x, &color.y, &color.z) == 3) {
                    mat.diffuse = color;
                }
                if (sscanf(line, "Ka %lf %lf %lf", &color.x, &color.y, &color.z) == 3) {
                    mat.ambient = color;
                }
                // break;
                // case 'K':
                if (sscanf(line, "Ks %lf %lf %lf", &color.x, &color.y, &color.z) == 3) {
                    mat.specular = color;
                }
            case 'R':
                if (sscanf(line, "Rf %f", &reflect) == 1) {
                    mat.reflect = reflect;
                }
            case 'O':
    ...
    void load_obj(const char *filename, float scale = 1) {
        ...
        while (fgets(line, 1000, fn) != NULL) {
            ...
            if (line[0] == 'v') {
                if (line[1] == ' ')
                    if (sscanf(line, "v %lf %lf %lf", &val.x, &val.y, &val.z) == 3) {
                        val.x *= scale;
                        val.y *= scale;
                        val.z *= scale;
                ...
                if (line[1] == 't')
                    if (sscanf(line, "vt %lf %lf", &val.x, &val.y) == 2)
                        geo.tex_coords.push_back(val);

                if (line[1] == 'n')
                    if (sscanf(line, "vn %lf %lf %lf", &val.x, &val.y, &val.z) == 3)
            ...
            if (line[0] == 'f') {
                ...
                if (sscanf(line, "f %d %d %d", &integers[0], &integers[1], &integers[2]) == 3) {
                    ...
                    tri_ok = true;
                                  ...
                                  &integers[3], &integers[1], &integers[7], &integers[4],
            ...
            if (line[0] == 'u') {
                ...
                if (sscanf(line, "usemtl %s", material_str) == 1) {
                    material =
                        get_material_index(std::string(mtl_prefix + std::string(material_str)));
